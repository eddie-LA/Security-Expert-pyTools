...
class Cameras:
    ...
    # Takes: 
    # Returns: 
    # Handle DVR and FloorPlan
    def ProcessIDs(self, dictdf_sites: pd.DataFrame, dictdf_fp: pd.DataFrame, dictdf_dvr: pd.DataFrame):
        ...
        if not dictdf_dvr.empty: # this should do nothing to entries with values 2147483647, but still check for other values and fix them appropriately
            self.tempdf['DVR'] = self.tempdf['DVR'].replace([dictdf_dvr['Old'].values], [dictdf_dvr['New'].values])
    ...
    def Restore(self, cnxn): # maybe name this Preprocess(), rather than Restore()
        ...
        df_list = [d for _, d in self.tempdf.groupby('CameraID')] 
        ...
        for index, df in enumerate(df_list):
            df_list[index] = (df_list[index].drop(columns=['CameraID'])
                                            .dropna(axis='columns')
            )
        ...
        self.dictdf = pd.DataFrame({'Old': self.tempdf['CameraID'].values, 
                                    'New': df_readback['CameraID'].values
                                    })
