...
class FloorPlanSymbols:
    table_name = 'FloorPlanSymbols'
    ...
    def Restore(self, cnxn): # maybe name this Preprocess(), rather than Restore()
        ...
        df_list = [d for _, d in self.tempdf.groupby('FloorPlanSymbolID')] 
        ...
        for index, df in enumerate(df_list):
            df_list[index] = (df_list[index].drop(columns=['FloorPlanSymbolID'])
                                            .dropna(axis='columns')
            )
        ...
        self.dictdf = pd.DataFrame({'Old': self.tempdf['FloorPlanSymbolID'].values, 
                                    'New': df_readback['FloorPlanSymbolID'].values
                                    })
