...
class Intercoms:
    ...
    # Takes:
    # Returns: 
    # Handle Sites, FPs, Cameras
    def ProcessIDs(self, dictdf_sites: pd.DataFrame, dictdf_fps: pd.DataFrame, dictdf_cams: pd.DataFrame):
    ...
    def Restore(self, cnxn): # maybe name this Preprocess(), rather than Restore()
        ...
        df_list = [d for _, d in self.tempdf.groupby('IntercomID')] 
        ...
        for index, df in enumerate(df_list):
            df_list[index] = (df_list[index].drop(columns=['IntercomID'])
                                            .dropna(axis='columns')
            )
        ...
        self.dictdf = pd.DataFrame({'Old': self.tempdf['IntercomID'].values, 
                                    'New': df_readback['IntercomID'].values
                                    })
