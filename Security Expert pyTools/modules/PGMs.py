...
class PGMs:
    ...
    # Takes: 
    # Returns: 
    # Handle Controllers, HostControllerRef (same as ControllerID?), FloorPlan, Camera | TODO: search for changed IDs in remote DB if not found in backup?
    def ProcessIDs(self, dictdf_ctrlrs: pd.DataFrame, dictdf_fps: pd.DataFrame, dictdf_cams: pd.DataFrame):
        ...
        # ----- Get info for any changes in ControllerID and HostControllerRef (should be = ControllerID most times) and apply any differences here | TODO: handle orphaned data?
        ...
        if not dictdf_cams.empty:
            self.tempdf['Camera'] = self.tempdf['Camera'].replace([dictdf_cams['Old'].values], [dictdf_cams['New'].values])
    ...
    def Restore(self, cnxn): # maybe name this Preprocess(), rather than Restore()
        ...
        self.dictdf = pd.DataFrame({'Old': self.tempdf['PGMID'].values, 
                                    'New': df_readback['PGMID'].values
                                    })
