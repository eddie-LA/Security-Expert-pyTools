...
class DVRs:
    ...
    # Takes: 
    # Returns: 
    # Handle DVR and FloorPlan (?) | TODO: search for changed IDs in remote DB if not found in backup?
    def ProcessIDs(self, dictdf_dvrs: pd.DataFrame):
        ...
        # ----- Get info for any changes in DVR and apply any differences here, will not touch 2147483647-valued entries | TODO: handle orphaned data?
        if not dictdf_dvrs.empty:
            self.tempdf['DVR'] = self.tempdf['DVR'].replace([dictdf_dvrs['Old'].values], [dictdf_dvrs['New'].values])
    ...
    def Restore(self, cnxn): # maybe name this Preprocess(), rather than Restore()
        ...
        self.dictdf = pd.DataFrame({'Old': self.tempdf['DVRID'].values, 
                                    'New': df_readback['DVRID'].values
                                    })
