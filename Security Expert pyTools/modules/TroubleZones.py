...
class TroubleZones:
    ...
    # Takes: 
    # Returns: 
    # Handle Controller and HostControllerRef, Areas, ZoneType (), TroubleGroup (), ReportingID (), FloorPlan, ModuleZone (?), ReportingService
    # handle ModuleType, it can be a Controller, Input, Door, etc
    def ProcessIDs(self, dictdf_ctrlrs: pd.DataFrame, dictdf_fp: pd.DataFrame, dictdf_areas: pd.DataFrame):
        ...
        if not dictdf_ctrlrs.empty:
            ...
            self.tempdf['HostControllerRef'] = self.tempdf['HostControllerRef'].replace([dictdf_ctrlrs['Old'].values], [dictdf_ctrlrs['New'].values])

        if not dictdf_fp.empty: # this should do nothing to entries with values 2147483647, but still check for other values and fix them appropriately
            self.tempdf['FloorPlan'] = self.tempdf['FloorPlan'].replace([dictdf_fp['Old'].values], [dictdf_fp['New'].values])

        if not dictdf_areas.empty: # this should do nothing to entries with values 2147483647, but still check for other values and fix them appropriately
            self.tempdf['Area1'] = self.tempdf['Area1'].replace([dictdf_areas['Old'].values], [dictdf_areas['New'].values])
            self.tempdf['Area2'] = self.tempdf['Area2'].replace([dictdf_areas['Old'].values], [dictdf_areas['New'].values])
            self.tempdf['Area3'] = self.tempdf['Area3'].replace([dictdf_areas['Old'].values], [dictdf_areas['New'].values])
            self.tempdf['Area4'] = self.tempdf['Area4'].replace([dictdf_areas['Old'].values], [dictdf_areas['New'].values])
    ...
    def Restore(self, cnxn): # maybe name this Preprocess(), rather than Restore()
        ...
        df_list = [d for _, d in self.tempdf.groupby('TroubleZoneID')] 
        ...
        for index, df in enumerate(df_list):
            df_list[index] = (df_list[index].drop(columns=['TroubleZoneID'])
                                            .dropna(axis='columns')
            )
        ...
        self.dictdf = pd.DataFrame({'Old': self.tempdf['TroubleZoneID'].values, 
                                    'New': df_readback['TroubleZoneID'].values
                                    })
