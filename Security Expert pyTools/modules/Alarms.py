...
class Alarms:
    ...
    # Takes: 
    # Returns: 
    # Handle Sites, FloorPlans, EventFilters
    def ProcessIDs(self, dictdf_sites: pd.DataFrame, dictdf_fp: pd.DataFrame, dictdf_evfilters: pd.DataFrame):
        ...
        if not dictdf_evfilters.empty: # this should do nothing to entries with values 2147483647, but still check for other values and fix them appropriately
            self.tempdf['EventFilter'] = self.tempdf['EventFilter'].replace([dictdf_evfilters['Old'].values], [dictdf_evfilters['New'].values])
    ...
    def Restore(self, cnxn): # maybe name this Preprocess(), rather than Restore()
        ...
        df_list = [d for _, d in self.tempdf.groupby('AlarmID')] 
        ...
        for index, df in enumerate(df_list):
            df_list[index] = (df_list[index].drop(columns=['AlarmID'])
                                            .dropna(axis='columns')
            )
        ...
        self.dictdf = pd.DataFrame({'Old': self.tempdf['AlarmID'].values, 
                                    'New': df_readback['AlarmID'].values
                                    })
