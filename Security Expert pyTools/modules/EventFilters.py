...
class EventFilters:
    table_name = 'EventFilters'
    date_fields=['StartDate', 'EndDate', 'LastModified', 'Created']
    ...
    # Takes: 
    # Returns: 
    # Handle Sites, EventGroups (only if custom ones can be created), FloorPlan, Group (?)
    # -- left to handle: EventGroups, Group (?)
    def ProcessIDs(self, dictdf_sites: pd.DataFrame, dictdf_fp: pd.DataFrame):
    ...
    def Restore(self, cnxn): # maybe name this Preprocess(), rather than Restore()
        ...
        df_list = [d for _, d in self.tempdf.groupby('EventFilterID')] 
        ...
        for index, df in enumerate(df_list):
            df_list[index] = (df_list[index].drop(columns=['EventFilterID'])
                                            .dropna(axis='columns')
            )
        ...
        self.dictdf = pd.DataFrame({'Old': self.tempdf['EventFilterID'].values, 
                                    'New': df_readback['EventFilterID'].values
                                    })
